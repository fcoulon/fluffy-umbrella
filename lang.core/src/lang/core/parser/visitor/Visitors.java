...
//			}
	...
	static class ClassVisitor extends XtdAQLBaseVisitor<ExtendedClass> {
		...
		public ClassVisitor(ParseResult<ModelBehavior> parseRes, ModelBehavior modelBehavior) {
			this.parseRes = parseRes;
	...
	static class ModelBehaviorVisitor extends XtdAQLBaseVisitor<ModelBehavior> {
		...
		public ModelBehavior visitRRoot(RRootContext ctx) {
			...
			ClassVisitor subVisitor = new ClassVisitor(parseRes, res);
			...
			res.getClassExtensions()
					.addAll(ctx.rClass().stream().map(cls -> subVisitor.visit(cls)).collect(Collectors.toList()));
